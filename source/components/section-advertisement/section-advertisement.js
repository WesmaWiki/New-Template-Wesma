	...
	function () {
		(function () {
			...
			function resize(e) {
				...
				screenHeight = canvas.height = canvas.parentNode.clientHeight;
			...
			function addParticle(num) {
				var i, p;
				for (i = 0; i < num; i++) {
					p = new Particle(Math.floor(Math.random() * screenWidth - PARTICLE_RADIUS * 2) + 1 + PARTICLE_RADIUS, Math.floor(Math.random() * screenHeight - PARTICLE_RADIUS * 2) + 1 + PARTICLE_RADIUS, PARTICLE_RADIUS);
			...
			canvas = document.getElementById("section-advertisement-main-canvas");
			...
			if (canvas !== null) {
				...
				var loop = function () {
					...
					bufferCtx.save();
					bufferCtx.fillStyle = bufferCtx.strokeStyle = "#000";
