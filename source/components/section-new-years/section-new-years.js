...
class World {
	...
	loop() {
		...
		if (this.analyser && this.isRunning) {
			...
			if (justFinished) {
				...
				this.audioBtn.textContent = "Воспроизвести снова";
	...
	addSpiral() {
		...
		const colorChoices = ["blue", "green", "Indigo", "DarkGreen", "red"];

		const squareGeometry = new THREE.CircleGeometry(1, 40);
	...
	addOctahedron({ color = "red", scale, position = [0, 0, 0] }) {
	...
	addOctahedrons() {
		...
		this.addOctahedron({ color: "YellowGreen", scale: [1, 1.4, 1] });
		this.addOctahedron({
			color: "DarkTurquoise",
			position: [0, 0.85, 0],
			scale: [0.5, 0.7, 0.5],
	...
	addButton() {
		...
		this.audioBtn.addEventListener("click", () => {
			...
			if (this.analyser) {
				...
				this.audioBtn.textContent = "Загрузка...";
				this.loadMusic().then(() => {
...
if (document.querySelector(".new-years__canvas") != null) {
	const world = new World({
		canvas: document.querySelector(".new-years__canvas"),
