...
if (document.getElementById("canvas") != null) {
	window.onload = function () {
		...
		class rope {
			constructor(tx, ty, l, b, slq, typ) {
				...
				for (let i = 1; i < this.res; i++) {
					this.segm.push(new segm(this.segm[i - 1].x1, this.segm[i - 1].y1, this.l / this.res));
				}
				this.b = b;
			}
			update(t) {
			...
			show() {
				if (this.type == "l") {
					...
					c.beginPath();
					c.arc(this.segm[this.segm.length - 1].x1, this.segm[this.segm.length - 1].y1, 2, 0, 2 * Math.PI);
					c.fillStyle = "#43bea2";
					c.fill();
				} else {
					for (let i = 0; i < this.segm.length; i++) {
		...
		for (let i = 0; i < 100; i++) {
			...
			ropes.push(new rope(w / 2, h / 2, (Math.random() * 1 + 0.5) * 500, Math.random() * 0.4 + 0.1, Math.random() * 15 + 5, type));
		...
		disabledAnim();
		match[0].addListener(disabledAnim);
	};
}
