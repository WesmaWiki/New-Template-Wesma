...
const { E, LN10, LN2, LOG10E, LOG2E, PI, SQRT1_2, SQRT2, abs, acos, acosh, asin, asinh, atan, atan2, atanh, cbrt, ceil, clz32, cosh, exp, expm1, floor, fround, hypot, imul, log, log10, log1p, log2, max, min, pow, /* random, */ round, sign, sinh, sqrt, tan, tanh, trunc } = Math;
...
let _canvasOptions = {};
let canvas = document.getElementById("canvas-wesma");
if (canvas === null) {
...
window.addEventListener("load", () => {
	...
	Object.assign(_canvasOptions, _defaulCanvasOptions, canvasOptions ? canvasOptions : {});
...
function _resizeCanvas(specificCanvas) {
	width = canvas.width = _canvasOptions.width !== null ? _canvasOptions.width : canvas.parentNode.clientWidth;
	height = canvas.height = _canvasOptions.height !== null ? _canvasOptions.height : canvas.parentNode.clientHeight;
...
function lerpRGB(...args) {
	...
	if (args.length === 3) {
		...
		[{ r: r1 = 255, b: b1 = 255, g: g1 = 255, a: a1 = 1 }, { r: r2 = 0, b: b2 = 0, g: g2 = 0, a: a2 = 1 }, t] = args;
...
function lerpAngle(start, end, amt = 0.5) {
	return atan2((1 - amt) * sin(start) + amt * sin(end), (1 - amt) * cos(start) + amt * cos(end));
}
...
const ease = {
	...
	linear: Object.assign(linearTween, { in: linearTween, out: linearTween, inOut: linearTween }),
...
function getFontSizeForTargetWidth(text, targetWidth) {
	...
	return min(((testSize / measurement.width) * targetWidth) / 1.4, 200);
}
...
let buttonSlideChange = document.querySelectorAll(".seo-notwork__button");

let seoNotworkSlider = new Swiper(".seo-notwork__slider", {
	slidesPerView: 1,
	watchOverflow: true,
	autoHeight: true,
	navigation: {
		nextEl: ".seo-notwork__next",
		prevEl: ".seo-notwork__prev",
	},

	on: {
		afterInit: function (swiper) {
			if (buttonSlideChange.length > 0) {
				buttonSlideChange[swiper.activeIndex].classList.add("active");
			}
		},

		slideChange: function (swiper) {
			buttonSlideChange.forEach((el) => el.classList.remove("active"));
			buttonSlideChange[swiper.activeIndex].classList.add("active");
		},
	},
});
...
if (buttonSlideChange.length > 0) {
	buttonSlideChange.forEach((element, index, array) => {
		element.addEventListener("click", function (e) {
			if (!element.classList.contains("active")) {
				array.forEach((el) => el.classList.remove("active"));
				element.classList.add("active");
				seoNotworkSlider.slideTo(index, 300, false);
			}
		});
	});
}
